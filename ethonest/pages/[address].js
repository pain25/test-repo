...
import Campaign from "../artifacts/contracts/campagin.sol/Campaign.json";
...
export default function Detail({ Data, DonationsData }) {
  ...
  const [inputUrl, setTextUrl] = useState("");
  ...
  useEffect(() => {
    const Request = async () => {
      ...
      const inputUrl = `https://gateway.pinata.cloud/ipfs/${Data.storyUrl}`;
      setTextUrl(inputUrl);
      //it takes input url and converts it in the form of which can be displayed in the page
        ...
        fetch(inputUrl)
          ...
          .then((html) => {
            if (html) {
              ...
              console.log("Empty HTML data received");
            }
          })
